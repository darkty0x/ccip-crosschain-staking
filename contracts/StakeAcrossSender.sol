...
/// @title - StakeAcross Sender contract for sending messages to other chains
/// @notice - This contract inherits from the CCIPReceiver contract, which allows it to receive messages from other chains.
/// @dev It also inherits from the OwnerIsCreator contract, which restricts access to certain functions to the contract owner.

contract StakeAcrossSender is CCIPReceiver, OwnerIsCreator {
    ...
    /// @dev This function is called by the depositor to send a message to another chain.
    /// @param destinationChainSelector The chain selector of the destination chain.
    /// @param receiver The address of the receiver contract on the destination chain.
    /// @param tokenToTransfer The address of the token to transfer.
    ...
    function sendMessage(
        ...
        //  depositor's EOA as data to be sent in the message.
        address depositor = msg.sender;

        Client.EVM2AnyMessage memory evm2AnyMessage = Client.EVM2AnyMessage({
            receiver: abi.encode(receiver), // ABI-encoded receiver contract address
            data: abi.encode(depositor), // ABI-encoded depositor's address
            tokenAmounts: tokenAmounts,
            extraArgs: Client._argsToBytes(
                Client.EVMExtraArgsV1({gasLimit: _gasLimit}) // Additional argument: gas limit
            ),
    ...
    /// @notice This function is called by the CCIP router when a message is received.
    ...
    // Utility functions to cleanup by withdrawing tokens: Native token, if any, LINK and BnM tokens.
    // These functions are not part of the protocol and are only used for testing purposes.
